...
void filler( uint8_t* temp, uint8_t * arr,int stride ){
...
/*Threading helper function that populates numpy array with pixel data*/
void filler_16(uint16_t* temp, uint16_t * arr, int stride) {
	//uint16_t* temp = (uint16_t *)start;
	//   uint8_t
	for (int i = 0; i<stride; i++) {
		temp[i] = arr[i];
	}
...
//    npy_intp s = PyArray_STRIDE (ret, 0);
	...
	if (dtype->type_num == NPY_UBYTE) {
		...
		status = false;
		std::cout << "That numpy data type is not supported\n";
	//	goto fail;
	}
...
//
	...
	if (status) {
		...
		if (flag) {
			...
			for (uint64_t i = 0; i < image_rows; i++) {
				temp_16 = (uint16_t*)PyArray_GETPTR2(ret, i, 0);
				threadder.push_back(std::thread(filler_16, temp_16, &data_arr_16[position], image_columns));
				position += image_columns;
			}
		}
		for (auto& thread : threadder) {
